...
const CASES: &[(u32, &str)] = &[
    ...
    (
        0x4533EE14,
        "aesdimc { z20.b, z21.b }, { z20.b, z21.b }, z16.q[2]",
    ),
    (
        0x453AEAEE,
        "aese    { z14.b, z15.b }, { z14.b, z15.b }, z23.q[3]",
    ),
    ...
    (
        0x4526EA14,
        "aese    { z20.b - z23.b }, { z20.b - z23.b }, z16.q[0]",
    ),
    (
        0x4527EE14,
        "aesdimc { z20.b - z23.b }, { z20.b - z23.b }, z16.q[0]",
    ),
...
fn sve_aes2_gated() {
    ...
    for w in [
        ...
        assert!(
            !decode(w, 0, yes).is_invalid(),
            "{w:08X} should decode with SveAes2"
        );
    }
    // The multi-vector narrowing converts are gated on Sve2p1, not AES2.
    for w in [0x4531430Fu32, 0x45314B0F, 0x4531530F] {
        assert!(
            decode(w, 0, FeatureSet::BASE.with(Feature::Sve)).is_invalid(),
            "{w:08X} needs Sve2p1"
        );
        assert!(
            !decode(w, 0, base).is_invalid(),
            "{w:08X} should decode with Sve2p1"
        );
    }
}
...
fn fprcvt_gated() {
    ...
    for w in [0x1EF40243u32, 0x1E7A0155, 0x1E75000B, 0x1E7C0143] {
        ...
        assert!(
            !decode(w, 0, yes).is_invalid(),
            "{w:08X} should decode with Fprcvt"
        );
    }
...
fn mops_and_tchange_gated() {
    ...
    for w in [0x1DDF0211u32, 0x1DDF4211, 0x1DDFB211] {
        assert!(
            decode(w, 0, FeatureSet::BASE).is_invalid(),
            "{w:08X} needs Mops"
        );
...
fn reserved_slots_stay_invalid() {
    ...
    let reserved: &[u32] = &[
        ...
        0x4520EA14, // AES2 pair base must be even (odd dst -> UNDEFINED).
        0x4522EA15, // FPRCVT: same-width pair (S->S) is UNALLOCATED in this slot.
        0x1E2A0143, // SETGO*: the value field `<20:16>` must be xzr (this has x0).
        0x1DC04211, // SETGO*: op2 12..15 are unallocated.
        0x1DDFC211, // TCHANGE: op0 (`<20:19>`) 01/11 are unallocated.
        0xD5A403E9, 0xD5E403E9, // TCHANGE register form: high CRm bits must be 0.
        0xD5840409,
    ];
    for &w in reserved {
        assert!(
            decode(w, 0, FeatureSet::ALL).is_invalid(),
            "{w:08X} should be Invalid"
        );
    }
}
