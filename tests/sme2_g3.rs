...
fn luti_zt0_examples() {
    ...
    assert_eq!(
        text(0xC09D9173),
        "luti2   { z19.h, z23.h, z27.h, z31.h }, zt0, z11[1]"
    );
    // Register-pair source: a 2-register group, no element suffix, no index.
    assert_eq!(
        text(0xC08B005C),
        "luti4   { z28.b - z31.b }, zt0, { z2, z3 }"
    );
    assert_eq!(
        text(0xC09B0000),
...
fn luti_zt0_roundtrip_sweep() {
    ...
    assert!(
        decoded >= 1000,
        "expected many LUTI ZT0 forms, got {}",
        decoded
    );
    assert!(strided_seen > 0, "expected strided LUTI forms in the sweep");
    assert!(
        pair_seen > 0,
        "expected register-pair-source LUTI forms in the sweep"
    );
}
...
fn luti_zt0_reserved_and_neighbours() {
    ...
    assert!(
        decode(l4_4b, 0, FeatureSet::ALL).is_invalid(),
        "LUTI4 4-reg .b must be Invalid"
    );
    // LUTI4 4-register `.h` (size 01) *is* valid (sanity that the size guard is
    // surgical).
    let l4_4h = 0xC088_0000 | (1 << 17) | (1 << 15) | (1 << 12);
    assert!(
        !decode(l4_4h, 0, FeatureSet::ALL).is_invalid(),
        "LUTI4 4-reg .h must decode"
    );
    ...
    assert!(
        zero_mnem != "luti2" && zero_mnem != "luti4",
        "ZERO region not LUTI"
    );

    // Reserved word<11:10> non-zero is UNDEFINED.
    let resv = 0xC0CC_0000 | (1 << 10);
    assert!(
        decode(resv, 0, FeatureSet::ALL).is_invalid(),
        "reserved word<10> must be Invalid"
    );

    // Element size word<13:12>=11 is unallocated.
    let bad_size = 0xC0CC_0000 | (0b11 << 12);
    assert!(
        decode(bad_size, 0, FeatureSet::ALL).is_invalid(),
        "size 11 must be Invalid"
    );

    // Multi-register destinations must be span-aligned.
    let odd2 = 0xC08C_4000 | 1; // luti2 {z1,z2} (odd base) — invalid
    assert!(
        decode(odd2, 0, FeatureSet::ALL).is_invalid(),
        "odd 2-reg base must be Invalid"
    );
    ...
    assert!(
        decode(str2_s, 0, FeatureSet::ALL).is_invalid(),
        "strided .s must be Invalid"
    );
    // LUTI4 4-register strided is `.h`-only (`.b` invalid).
    let str4_l4_b = 0xC088_0000 | (1 << 20) | (1 << 17) | (1 << 15); // strided L4 4-reg .b
    assert!(
        decode(str4_l4_b, 0, FeatureSet::ALL).is_invalid(),
        "L4 4-reg strided .b Invalid"
    );
    ...
    assert!(
        decode(str2_badbase, 0, FeatureSet::ALL).is_invalid(),
        "strided 2-reg base z8 Invalid"
    );
    ...
    assert!(
        decode(single_strided, 0, FeatureSet::ALL).is_invalid(),
        "single-reg strided Invalid"
    );
    ...
    assert!(
        !pi.is_invalid() && pi.mnemonic().name() == "luti4",
        "pair-source form must decode"
    );
    assert!(
        matches!(
            pi.op(2),
            fARM64::Operand::SveVecGroup {
                count: 2,
                arr: None,
                ..
            }
    ...
    assert!(
        decode(pair_oddsrc, 0, FeatureSet::ALL).is_invalid(),
        "odd pair-source base Invalid"
    );
}
...
fn luti_zt0_feature_gated() {
    ...
    for w in [0xC08C407Au32, 0xC0CC0000, 0xC0CA0000, 0xC08A9000] {
        assert!(
            decode(w, 0, no_lut).is_invalid(),
            "{:08X} should require FEAT_LUT",
            w
        );
        assert!(
            !decode(w, 0, FeatureSet::ALL).is_invalid(),
            "{:08X} should decode with FEAT_LUT",
            w
        );
    }
}
...
fn za_tile_move_examples() {
    ...
    assert_eq!(
        text(0xC0040183),
        "mov     za0h.b[w12, 6:7], { z12.b, z13.b }"
    );
    assert_eq!(
        text(0xC0040400),
    ...
    assert_eq!(
        text(0xC0860600),
        "movaz   { z0.s - z3.s }, za0h.s[w12, 0:3]"
    );
}
...
fn za_tile_move_roundtrip_sweep() {
    ...
    assert!(
        decoded >= 500,
        "expected many ZA tile-move forms, got {}",
        decoded
    );
}
...
fn za_tile_move_reserved_and_neighbours() {
    ...
    assert!(
        decode(0xC0040000 | (1 << 16), 0, FeatureSet::ALL).is_invalid(),
        "word<16> reserved"
    );
    // word<12>=1 / word<11>=1 are reserved → Invalid.
    assert!(
        decode(0xC0040000 | (1 << 12), 0, FeatureSet::ALL).is_invalid(),
        "word<12> reserved"
    );
    // word<11>=1 is the *array-vector* MOV neighbour (`za.d[w8, ..]`) — out of
    // scope, must not be claimed as the tile-slice form.
    assert!(
        text(0xC0040800) != "mov     za0h.b[w12, 0:1], {z0.b, z1.b}",
        "array-vector MOV not tile form"
    );
    // word<19>=1 is the ZERO neighbour — must not be claimed.
    assert!(
        decode(0xC0080000, 0, FeatureSet::ALL).mnemonic().name() != "mov",
        "ZERO not MOV"
    );

    // Misaligned vgx2 register group base (odd) is UNDEFINED.
    assert!(
        decode(0xC0040000 | (1 << 5), 0, FeatureSet::ALL).is_invalid(),
        "odd vgx2 base Invalid"
    );
    // Misaligned vgx4 register group base (not multiple of 4) is UNDEFINED.
    assert!(
        decode(0xC0040400 | (2 << 5), 0, FeatureSet::ALL).is_invalid(),
        "vgx4 base %4 Invalid"
    );
    // Out-of-range slice offset (vgx4 `.b`, offset field 4) is UNDEFINED.
    assert!(
        decode(0xC0040400 | 4, 0, FeatureSet::ALL).is_invalid(),
        "vgx4 .b offset 16 Invalid"
    );
    ...
    assert!(
        decode(0xC0060000 | (1 << 8), 0, FeatureSet::ALL).is_invalid(),
        "za->v word<8> reserved"
    );
    // vectors->ZA reserved word<4:3> non-zero is UNDEFINED.
    assert!(
        decode(0xC0040000 | (1 << 3), 0, FeatureSet::ALL).is_invalid(),
        "vza word<3> reserved"
    );
}
...
fn za_tile_move_feature_gated() {
    ...
    for w in [0xC0040183u32, 0xC0060200, 0xC0040400, 0xC0860600] {
        assert!(
            decode(w, 0, no_sme2).is_invalid(),
            "{:08X} should require FEAT_SME2",
            w
        );
        assert!(
            !decode(w, 0, FeatureSet::ALL).is_invalid(),
            "{:08X} should decode with FEAT_SME2",
            w
        );
    }
}
