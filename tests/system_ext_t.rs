...
fn feature_gating() {
    ...
    assert_eq!(
        decode(0xD50B7700, ADDR, g).mnemonic(),
        fARM64::Mnemonic::Gcspushm
    );
    assert_eq!(
        decode(0xD91F0C20, ADDR, g).mnemonic(),
        fARM64::Mnemonic::Gcsstr
    );
    // TENTER is gated on FEAT_TEV (not GCS).
    assert!(decode(0xD4E004C0, ADDR, g).is_invalid());
    let tev = FeatureSet::BASE.with(Feature::Tev);
    assert_eq!(
        decode(0xD4E004C0, ADDR, tev).mnemonic(),
        fARM64::Mnemonic::Tenter
    );
    // CHKFEAT is gated on FEAT_CHK, SHUH on FEAT_PCDPHINT.
    assert_eq!(
        decode(0xD503251F, ADDR, FeatureSet::BASE).mnemonic(),
        fARM64::Mnemonic::Hint
    );
    assert_eq!(
    ...
    assert_eq!(
        decode(0xD508A94A, ADDR, FeatureSet::BASE).mnemonic(),
        fARM64::Mnemonic::Plbi
    );
}
